...
function handleInstall(appPath) {
    ...
    if (fs.pathExistsSync(templateDependenciesPath)) {
        ...
        installVendors(pkgToInstall, projectCustom.command, true).then(
            result => {
                // 再装dependencies
                ...
                if (dependencies) {
                    ...
                    installVendors(pkgToInstall, projectCustom.command, false).then(
                        result => {
                            finishCreate(appPath, projectCustom.name, projectCustom.command);
                        }, err => {
    ...
    return inquirer
        .prompt([
            ...
            },
            {
                ...
                message: `${chalk.cyan.bold('请选择要使用的包管理工具？')}`,
                default: 'npm'
            }
        ])
        .then((answers) => {
            if (answers.command === 'yarn' && !shouldUseYarn()) {
                spinner.fail(`${chalk.red.bold(`您还未安装 ${chalk.blue.bold('yarn')} 包管理工具，请先完成安装之后再创建项目~`)}\n${chalk.yellow.bold('→_→')}\n`);
                process.exit(1);
            } else if (answers.command === 'cnpm' && !shouldUseCnpm()) {
                spinner.fail(`${chalk.red.bold(`您还未安装 ${chalk.blue.bold('cnpm')} 包管理工具，请先完成安装之后再创建项目~`)}\n${chalk.yellow.bold('→_→')}\n`);
                process.exit(1);
            } else if (answers.command === 'npm' && !shouldUseNpm()) {
                spinner.fail(`${chalk.red.bold(`您还未安装 ${chalk.blue.bold('npm')} 包管理工具，请先完成安装之后再创建项目~`)}\n${chalk.yellow.bold('→_→')}\n`);
                process.exit(1);
            }
