...
function handleInstall(appPath, command) {
    ...
    if (fs.pathExistsSync(templateDependenciesPath)) {
        ...
        installVendors(pkgToInstall, command, true).then(
            result => {
                // 再装dependencies
                ...
                if (dependencies) {
                    ...
                    installVendors(pkgToInstall, command, false).then(
                        result => {
...
async function createApp(name) {
    ...
    handleInstall(appPath, projectCustom.command);
}
