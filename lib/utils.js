...
const installVendors = promisify((packageToInstall, command, saveDev, callback) => {
    let args, saveAddArg, saveDevArg, saveExactArg;

    if (command === 'yarn' && shouldUseYarn()) {
        saveAddArg = 'add';
        saveDevArg = saveDev ? '--dev' : undefined;
        saveExactArg = '--exact';
    } else if (command === 'cnpm' && shouldUseCnpm()) {
        saveAddArg = 'install';
        saveDevArg = saveDev ? '--save-dev' : '--save';
        saveExactArg = '--save-exact';
    } else {
        saveAddArg = 'install';
        saveDevArg = saveDev ? '--save-dev' : '--save';
        saveExactArg = '--save-exact';
    ...
    args = [saveAddArg, saveDevArg, saveExactArg].concat(packageToInstall);
...
const finishCreate = async (root, name, command) => {
    console.log();
    ...
    console.log(chalk.cyan.bold(`${command} start`));
    console.log('> 启动本地服务，进行开发');
    console.log(chalk.cyan.bold(`${command === 'yarn' ? command : `${command} run`} build:dev`));
    console.log('> 构建测试包，部署测试');
    console.log(chalk.cyan.bold(`${command === 'yarn' ? command : `${command} run`} pack`));
